...
def run_static_detection(df):
    ...
    for level in levels:
        ...
        for level_removed in range(0, 4):

            duplicate_groups_by_name_and_partial_hierarchy = find_similar_groups_with_hierarchy_level(df_duplicates_by_name, level_removed)
            df_duplicates_by_name_and_partial_hierarchy = get_rows(df_duplicates_by_name, duplicate_groups_by_name_and_partial_hierarchy)
            df_duplicates_by_name_and_partial_hierarchy['LEVELS_REMOVED'] = f'{level_removed} LEVELS'

            # Concatenate the dataframes
            merged_df_with_levels = pd.concat([merged_df_with_levels, df_duplicates_by_name_and_partial_hierarchy])

            # Drop duplicates, keeping the first occurrence
            merged_df_with_levels = merged_df_with_levels.drop_duplicates(subset=['NAME', 'CODE', 'HIERARCHY', 'IS_GROUP', 'CHILDREN', 'LEVEL_NAME'])

            # Reset index for better readability
            merged_df_with_levels = merged_df_with_levels.reset_index(drop=True)
